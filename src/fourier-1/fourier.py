...
class FourierCirclesScene(Scene):
    ...
    def setup(self):
        ...
        print(f"slow_factor: {self.slow_factor}")
        Scene.setup(self)
    ...
    def get_y_component_wave(self,
        ...
        print('path: ', path, path.get_start(), path.get_end())
        wave = ParametricFunction(
            lambda t: op.add(
                right_shift_rate * t * LEFT,
                path.function(t)[1] * UP
            ),
            t_range=( path.get_start(), path.get_end(), 0.01),
            color=color,
        )
        wave_copies = VGroup(*[
            wave.copy()
...
class FourierOfPiSymbol(FourierCirclesScene):
    ...
    def setup(self):
        ...
        config = {**FourierOfPiSymbol.CONFIG, **type(self).CONFIG}
        for key, value in config.items():
            setattr(self, key, value)
...
class CustomFourierScene(FourierOfPiSymbol):
    ...
    def get_shape(self):
        svg_path = script_dir / 'drawing.svg'
        console.log('self.svg_path: ', svg_path)
